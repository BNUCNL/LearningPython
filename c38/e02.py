...
def accessControl(failIf):
    def onDecorator(aClass):
        if not __debug__:  # __debug__一般默认是true的，但是以-O选项执行该脚本时,__debug__就会变成false（话说-O选项具体是什么意思？）
            return aClass
        else:
            class onInstance:
...
# X.age = 999  # 同上，报错是因为不以-O选项运行该脚本时__debug__为true，则装饰后的Person指向的是类onInstance，而属性age又被设定成了“私有”属性
# print(X.age)  # 同上
